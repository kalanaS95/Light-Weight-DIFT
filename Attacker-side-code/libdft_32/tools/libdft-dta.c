...
void init_IPC(void) {
	...
	if(shmid < 0) {
		perror("shmget: IPC creation failed \n");
		exit(EXIT_FAILURE);	
	}	
	
	shm = (char *)shmat(shmid, NULL, 0);
	if(shm == (char*) -1) {
		perror("shmat: Error accessing shared memory");
		exit(EXIT_FAILURE);		
	}
}
...
void transmit_msg(char* msg) {
	...
	// we need to wait till the pipe is clear
	
	while(*shm != 0) {
		usleep(10); //this sleep time allow this producer from accessing the shared memory, so that consumer has access to clean the memory
	}
	
	memcpy(shm, msg, msg_len);
	s = shm;
	s += msg_len;
	*s = 0;
}
...
{
	/*This buffer is used to copy IPC messages*/
	char msg_buffer[SHSIZE];
	/* log file */
	FILE *logfile;
	...
	if (likely((logfile = fopen(logpath.Value().c_str(), "a")) != NULL)) {
		...
		(void)fprintf(logfile, "Process ID: [%d]\nOffending instruction address: [0x%08x]\nBranch target address: [0x%08x]\n",
							getpid(), ins, bt);
		snprintf(msg_buffer, SHSIZE, "Process ID: %d - Offending instruction address: 0x%08x - Branch target address: 0x%08x",
				getpid(), ins, bt);
	...
	char user_buffer[4];
	/* to terminate while loop below */
	bool terminate = false;
	
	printf("|------------------------------------------------------------------|\n");
	printf("| Suspicious instruction found at memory address   : 0x%08x    |\n", ins);
	printf("| Supsicious instruction will branch to instruction at: 0x%08x |\n",bt);
	printf("|------------------------------------------------------------------|\n\n");
	printf("Allow type [Yes] deny type [No]: \t");
	
	while(terminate == false)
	{
		scanf("%s", user_buffer);
		
		if(strcmp(user_buffer, "Yes") == 0 || strcmp(user_buffer, "No") == 0)
			terminate = true;
		else
			printf("\nAllow type [Yes] deny type [No]: \t");
	}
	
	if(strcmp(user_buffer, "No") == 0) {
		/* terminate */
		transmit_msg((char*)"Analysis process terminated by DIFT due to violations");
		transmit_msg((char*)"**TERMINATE**");
		printf("Program terminated by the user\n");
		exit(EXIT_FAILURE);
	}else {
		printf("Permissions granted by the user for further execution\n");
		transmit_msg((char*)"Permissions granted by the user for further execution");
	}
...
main(int argc, char **argv)
{
	...
	snprintf(msg_buffer, SHSIZE, "INIT: %d:%s:%s", process_ID, argv[10], argv[9]);
	transmit_msg(msg_buffer);
...
err:	/* error handling */
	...
	transmit_msg((char*)"Error occured while initializing PIN");
	libdft_die();
	/* return */
	return EXIT_FAILURE;
}
